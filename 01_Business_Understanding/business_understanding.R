...
# department /  job role subset
dept_job_role_tbl <- train_raw_tbl %>%
  select(EmployeeNumber, Department, JobRole, PerformanceRating, Attrition)

print(dept_job_role_tbl)
...
# Define Objectives: Retain High Performances
# Assess Outcomes: TBD
...
# Develop KPIs: Industry KPIs -- 8.8% benchmark
...
calculate_attrition_cost <- function(
  ...
  # Direct Costs
  direct_cost <- sum(separation_cost, vacancy_cost, 
                     acquisition_cost, placement_cost)
  
  # Lost Productivity Costs
  productivity_cost <- net_revenue_per_employee / workdays_per_year * 
    (workdays_position_open + workdays_onboarding * onboarding_efficiency)
  
  # Savings of Salary & Benefits (Costs Reduction)
...
dept_job_role_tbl %>%
  ...
  mutate(cost_text = str_c("$", 
                           format(cost_of_attrition / 1e6, digits = 2), 
                           "M", 
                           sep = "")) %>%
  ...
  labs(title = "Estimated Cost of Attrition",
       subtitle = "By Department and Job Role",
       x = "Cost of Attrition", y = "") +
  theme(legend.position = "none")


plot_attrtion <- function(data, ..., .value,
                          fct_reorder = TRUE,
                          fct_rev = FALSE,
                          include_lbl = TRUE,
  ...
  data_manipulated <- data %>% 
    ...
    mutate(value_text = str_c(usd(cost_of_attrition / units_val), 
                             units[[1]], sep = ""))
...
dept_job_role_tbl %>%
  ...
  plot_attrtion(Department, JobRole, .value = cost_of_attrition, 
                units = "M") +
  labs(title = "Estimated Cost of Attrition",
       subtitle = "By Department and Job Role",
       x = "", y = "",
       caption = "Source: Business Science")
